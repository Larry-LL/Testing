...
class PotentialFieldPlanner:
    ...
    def attractive_force(target, current):
        ...
        att_grad = (current - target) 
        att_f = -att_coef*att_grad 
    ...
    def repulsive_force(obstacle, current, unitvec=np.zeros((3,1))):
        ...
        if dist > Repulse_dist:
            ...
            rep_f = -rep_coef * (1/dist - 1/Repulse_dist)/ (dist**2) * unit.T
    ...
    def compute_torques(joint_forces, q):
        ...
        for i in range(7):
            torque = Jv[:i+1,:] @ joint_forces[:,i]
            joint_torques[:,:i+1] = joint_torques[:,:i+1] + torque 
        ## END STUDENT CODE
    ...
    def compute_gradient(q, target, map_struct):
        ...
        Force = PotentialFieldPlanner.compute_forces(target_joints.T , obstacle , current_joints.T)

        dq = PotentialFieldPlanner.compute_torques(Force , q ).reshape(1,7)
    ...
    def plan(self, map_struct, start, goal):
        ...
        while True:
            ...
            if self.q_distance(goal[:7], q[:7]) < self.tol or step >= self.max_steps: # TODO: check termination conditions
...
if __name__ == "__main__":
    ...
    # inputs 
    map_struct = loadmap("C:/Users/lusiy/OneDrive/Desktop/meam520_labs-main (1)/meam520_labs-main/maps/map2.txt")
